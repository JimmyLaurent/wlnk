...
function ImageParser(img, cb, options) {
  ...
  if (options.callbackEveryStep) {
    // show after every process, if we should
...
ImageParser.prototype.extract = function ExtractLetters(imgData) {
  ...
  for (var x = 0, j = imgData.width; x < j; ++x) {
    // for every column
    var foundLetterInColumn = false;

    for (var y = 0, k = imgData.height; y < k; ++y) {
      // for every pixel
      var pixIndex = (y * imgData.width + x) * 4;
      if (imgData.data[pixIndex] === 255) {
